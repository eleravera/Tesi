...
\section{Tracking in HEP}
    ...
    \subsection{Hybrid pixels at LHC and at SuperKEKB}
        ...
        \subsubsection{CMS}
        \red{da scrivere} hybrid pixel detector
...
with on-chip ADCs and digital readout at higher rate
    ...
    \subsubsection{BelleII}
        \red{due to the nanobam hai background, non puoi usare le strip in the innermost layer}
...
scattering for a precise reconstruction of B-decay vertices at our energies.
    ...
    \subsubsection{MIMOSA at EUDET and STAR}
        ...
        \centering
            ...
            \label{fig:}
        \end{figure}
        \red{epilayer not fully depleted, charge collection mostly by diffusion, tipical collection time bigger than 100 ns. Correlated Double Sampling}
        ...
        The readout is done in a rolling shutter mode: the chip is an Active Pixels (APS) and therefore it incorporates the amplification on pixel, while the signal discrimination and zero-suppression (\red{primo chip with integrated zero suppression}) logic are placed at the EoC, where is also placed a memory.
