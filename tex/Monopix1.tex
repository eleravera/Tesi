...
\section{Readout logic}
    ...
    \subsection{Dead time measurement}
        ...
        %t_reading = 25 ns * (ToT + CONF_START_FREEZE + N_pixels_to_read * (CONF_STOP_FREEZE - CONF_START_READ))
        %CONF_STOP_FREEZE - CONF_START_READ è il tempo che ci vuole a prendere i dati da un pixel e mandarli all'FPGA, e sono 34 cicli (850 ns). Se anche il ToT fosse zero (e non lo sarà) e riducessimo CONF_START_FREEZE a 3 (valore di default), per leggere 5 pixel ci vorrebbero 4.325 μs, che è più della durata di un pulse del fascio. Ridurre il tempo morto di Monopix1, dal punto di vista del fascio della FLASH, è una battaglia persa.



        To test the linearity of the dead time with the rate, I have 
