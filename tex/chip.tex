Pixel detectors are members of the semiconductor detectors and their operation is based on the creation of electron-hole couples in the bulk by the particle impinging which are then separated and drifted by the electric field and collected at their respectively electrodes.
...
\section{Signal formation}
   ...
   \begin{figure}[h!]
      ...
      \caption{The structure of a p-n junction. (a) structure, (b) space charge density, (c) electric field distribution and (d) potential
      distribution.

      A p-n junction is built by bringing in contact two n and p doped silicon crystals. At the boundary of the two doped region the majority carriers diffuse to the opposite part where they recombine, therefore, forming a region depleted of free carriers and with space charge instead. The charged donors$^{+}$ and acceptor$^{-}$, that remain in the n-type and p-type regions, create an electric field across the junction. 
    The majority charge carrier diffusion is opposed by the generated electric field and the junction reaches an equilibrium state.         
      
}
      \label{fig:junction}
  \end{figure}

   The width of the depletion region is related with the external bias $V_{ext}$, the resistivity $\rho$ and also with the dopant:
   \begin{multicols}{2}
       \begin{equation}
   ...
   The charges crated are then separated by the electric field and collected at their respectively electrodes ($p$ for holes and $n$ for electrons)\footnote{Even if in principle both the electrode can be used to read the signal, for pixel detectors, where the number of channel and the complexity of readout are high, only one is actually used. In strip and pad detectors, instead, is more common a dual-side readout}; by the drift of these charges, a signal $i_e$ is generated on the  electrode $e$ as stated by the Shockley-Ramo's theorem: 
    \begin{equation}
        i_e(t) = -q\: v(t)\, E_{WF,e}
    \end{equation}
...
\section{CCDs}
\red{descrivi come sono fatte e come funziona il readout}
Tens of \si{ms} due to the need to transfer the charge signals pixel by pixel through a single output circuit
For photon imaging the need of high assorbtion efficiency, \red{per cui usi materiali con alto Z}

\section{Hybrid pixels}
   \red{METTI IN EVIDENZAZ CHE PUOI FARE UN READOUT CON TECNOLOGIA CMOS. Metti in evidenza che sono più veloci}
...
\section{CMOS MAPS and DMPAS}
   ...
   \subsection{DMAPS: large and small fill factor}\label{sec:small-large-fill-factor}
      ...
      The small fill-factor variant, instead, benefits from a small capacity (\SIrange{5}{20}{fF}), but suffers from a not uniform electric field and from all the issue related to that. \red{Ho già detto prima parlando dei MAPS, devo ripetere qui?}\\
