...
\section{TJ-Monopix1 characterization}
    \subsection{Threshold and noise: figure of merit for pixel detectors}
        ...
        The mean minimum stable threshold evolved through different generation of chips: in the 1st generation it was around \SI{2500}{e-} while in the 3rd (corresponding to nowadays chips) is less than \SI{500}{e-}. This allows in thinner sensors with smaller signals: from \SI{16000}{e-} produced in \SI{200}{\um}, the signal expected moved down to \SI{2000}{e-} produced in \SI{25}{\um}. \red{According with this, the threshold of TJ-Monopix1 is around \SI{500}{e-}.}
        ...
        Thus, the noise sets a lower bound to the threshold: \red{if an occupancy $\leqslant$ $10^{-4}$ is required, for example, this correspond to the Gaussian 1-sided tail fraction for \SI{3.7}{\sigma}}.
        ...
        Therefore I perform a fit of the counts detected using the function in equation \ref{eq:fit_scurve}. In figure \ref{fig:scurve} there is an example with IDB equal to \SI{40}{DAC} of fit for a pixel belonging to the flavor B, while in table \ref{tab:Flavor_PMOS_reser} and figure \ref{fig:threshold_noise_hist} there are the histograms and the maps of the parameters of the scurve-fit. 
        \begin{figure}[h!]
            \centering
    ...
    \subsection{Calibration of the ToT}
        ...
        Finding a calibration for the ToT means defining a way to pass from the ToT values (0-64 clock counts) to a spectrum express in electrons collected. 
         The principles of the calibration are the sequent: the ToT in clock counts maps (eq.\ref{eq:}) a DAC range between the threshold and a value which specifically depends on the pixel and generally is around 90-100\si{DAC}. Assuming that a \SI{0}{DAC} signal corresponds to \SI{0}{e-}, if another reference point is fixed, a linear calibration function can be defined. 
        ...
        Moreover, considering that the charge injected in the FE goes to fill capacitor which is different from pixel to pixel, the true charge injected does not correspond to what expected assuming C equal to \SI{230}{aF}, which is the nominal value. 
        Accordingly to that, a measurement of this value provides both an absolute calibration of this capacity C and a conversion factor K to have a correspondence of the DAC signal in electrons. 
        K and C are defined respectivelly as:
        \begin{equation}
        ...
        \end{equation}
        Assuming the nominal value of C equal to \SI{230}{aF}, K is expected to be \SI{20}{e-/DAC}. 
        ...
        I recall indeed that there is a distinction in the structure of the low dose-epi layer (\ref{chap:}) among the rows, in particular pixels in rows 112-224, which have a RDPW are supposed to have a higher efficiency in the pixel corner. 
        ...
        While I have selected only a small region around the peak in the simple gaussian fit, with the eq.\ref{eq:} I have kept a part of the events in the long tail on the left: in particular, in both cases I have used an iterative routine to estabilish the better interval.
        ...
        The resolution of the detector of measuring the peak value, which is expected to be affected by the statistical fluctuation in the number of charge carries generated in the detector and by ENC, can be defined as the Fe55 peak width. 
        Then the width is expected to be:
        \begin{equation}
            \sigma_{Fe} = \sqrt{ENC^2 \,+ \,FN}
        \end{equation}
        Since the number of e/h pairs produced in the sensor is 1616, recalling that K for silicon detector is 0.115 and the ENC measured with the injection is \SI{12}{e-}, the $\sigma_{Fe}$ is expected to be $\sim$\SI{18}{e-}. Looking at figure \ref{fig:Fe55_resolution} the resolution achieved with the Fe55 source seems to be much higher. A contribution we have not taken into account but is certainly relevant is the systematic overestimation of the standard deviation of the Fe55 peak: this, as I already explained, is principally due to the high background of incomplete charge collection, which broadens the fitted peak. 
         The value of the capacity and the conversion factor found with the calibration are shown in \ref{fig:capacity_map}. The evident stripe-structure in the matrix has not a \red{non ha una motivazione certa, ma può derivare da come è fatta la slope, in cui si intravedono queste bande.}
        \begin{figure}[h!]
            \centering
            \includegraphics[width=.5\linewidth]{figures/charaterization/sigma_fe55.pdf}            
            \label{fig:Fe55_resolution}
            \caption{Distribution of the Fe55 width at the peak position (\SI{1616}{e-}), converted in electrons. }
        \end{figure}  
        \begin{figure}[h!]
            \centering
            \includegraphics[width=.7\linewidth]{figures/charaterization/conversion_factor_map.pdf}
            \caption{Map of the capacity (calibrated) at the input of the injection circuit. }
            \label{fig:capacity_map}
        \end{figure}  

        %\begin{figure}[h!]
        %    \centering
        %    \includegraphics[width=.49\linewidth]{figures/charaterization/max_tot_in_ele_hist.png}
        %    \includegraphics[width=.35\linewidth]{figures/charaterization/max_tot_in_ele_map.png}            
        %    \label{fig:max_charge_possible}
        %    \caption{}
        %\end{figure}   

        
        \subsubsection{HV flavor: an attempt of calibration}
        Despite an attempt of calibrating also the HV flavor, which is the  most different from the PMOS B flavor, has been done, we were unable because of a loss of signal due to the high capacity. \red{Non si vede il picco, cosa già difficile negli altri flavor e che ha richiesto molte ore di presa dati.}(figure \ref{fig:Fe55_spectrumHV}).  \red{spettro del ferro con HV}
    ...
    \subsection{Changing the bias}\label{chap:characterization_section:bias}
        ...
        The thickness of the depletion has to be considered indeed an important parameters for the efficiency of the signal, and in particular it affects the charge released by a particle which cross the sensor (since the signal is proportional to the thickness of the epitaxial layer).
         A 2D map of the measured output voltage amplitude and resulting gain in the case of the PMOS and HV are reported in figure \ref{fig:gain_vs_bias}
        Given that the chip under examination has a gap in the low dose epi-layer (look at chapter \ref{chapter:TJMonopix1_thesensor}) we were not able to change independently the bias of the substrate (PSUB) and of the p-well (PWELL), but they must be kept at the same value, differently from other chips of the same submission.
        Turning down the bias the depletion region is expected to narrow and the efficiency to reduce, especially in the pixel corner, raising the threshold and the noise and decreasing the slope, as a consequence of the reduction in the gain.
        \begin{figure}[h!]
            \centering
    ...
    \begin{table}
        ...
        \begin{figure}[h!]
            ...
            \caption{(a) Fe55 spectrum at different P$_{sub/well}$ bias. The ToT recorded values have been calibrated as explained in the above section. (b) The trend of the attenuations of the peak, the normalization and the rate with the bias are shown. Since during the collection of the whole data the source has been moved, it is not guaranteed that it has always had a repositioning in the same exactly place, then small the fluctuation of the rate along the decreasing trend are accettable. To evaluete the peak position and the normalization I have fit the spectrum with a gaussian in the region near the peak. }
    ...
    \subsection{Measurements with radioactive sources}
        ...
        In order to completely validate the operation of the whole sensor\footnote{As I will explained in chapter \ref{chap:} these measurements will foundamental also to be compared with the spectrum seen at the testbeam}, I have made some acquisitions with radioactive source, in particular I have used Fe55, Sr90Y, which is a $\beta^-$ emettitor with electron endpoint at \red{\SI{2}{MeV}}, and cosmic rays.
        ...
        \end{equation}
        Looking at the form of the histogram of the dimension, generally the Sr90 and the cosmic rays produce bigger clusters and hit a higher number of pixels. A reasonable motivation of this can be given considering that the two types of event are generated by two different particles: in the Sr90 and cosmic rays events the signals are supposed to be generated by electrons (and in the cosmic rays evets also by other charge particles); the Fe55 events, instead, are generated by a photo-electron which convert in the epitaxial layer. Depending on the position where it converts, the charge produced can be assorbed whithin more pixels of by only one. Anyway the number of pixels involved in the charge sharing is supposed to be reduced. In the other cases, the particles which generate the signal are passing through the detector, and their tracks across the matrix can be identified.
        \begin{figure}[h!]
            \centering
        ...
        The signal generated by electrons is similar to the one generated by cosmic rays \red{dovrei mettere qualche conto per giustificare questa affermazione}, and the spectrum is expected to follow a Langau-Gauss distribution.
        \red{nelle acquisizioni dei CR ho selezionato solo i cluster, per tagliare via il rumore.}
        \red{MODELLINO DEL RILASCIO DI CARICA SU PIU PIXEL}
    \begin{table}
            \begin{center}
            \begin{tabular}{| c |  c | c | c |}
            \hline
            Pixel per evt & Expected [e-] & Measured [e-]   \\
            \hline
            \hline
            1 & & \\
            1 & & \\ 
            ...
            \hline
            \end{tabular}
            \caption{Scaling proved with the events coming from the Sr90}
            \label{tab:}
            \end{center}
        \end{table}  
        
        VALORI PER MMODELLINO:
        mean = [1003.8398, 1555.1948, 2156.175, 2741.668] pm [31.683432 39.43596  46.43463  52.36094 ]
        max = [[ 950.] [1450.] [2050.] [2450.]] pm [22.403399 22.768036 23.217813 23.418407] 28.86751345948129. 
...
sungle pixel= [501.91226, 518.3834, 539.0669, 548.4218] pm [22.403399 22.768036 23.217813 23.418407]
    ...
    \subsection{Dead time measurements}
        ...
        Since the pre-amplifier response has a characteristic time $\sim$ToT, the dead time $\tau_{a}$ introduced by it will be at most \SI{1.6}{\us}; using the IRESET and VRESET FE parameters the reset time can be lowered down, but a \red{IRESET, puoi diminuire il tempo di scarica ma deve comunque}   
