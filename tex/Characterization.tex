...
\section{TJ-Monopix1 characterization}
    ...
    %python3 -i acquisition_Fe55/fit_tot_single_pixel.py -f acquisition_Fe55/source_PMOSS/Fe_acquisitions_6V/ per fare i fit. Attenzione che prende i file degli istogrammi npz già    
        ...
        2D maps of the value of the capacity and of the conversion factor found are shown in \ref{fig:capacity_map}. The evident stripe-structure in the matrix has not a \red{non ha una motivazione certa, ma può derivare da come è fatta la slope, in cui si intravedono queste bande.}
        \begin{figure}[h!]
            \centering
            \includegraphics[width=.5\linewidth]{figures/charaterization/sigma_fe55.pdf}            
            \caption{Distribution of the Fe55 peak width.}
            \label{fig:Fe55_resolution}
        \end{figure}  
        \begin{figure}[h!]
            \centering
            \includegraphics[width=.6\linewidth]{figures/charaterization/conversion_factor_map.pdf}
        ...
        \subsubsection{HV flavor calibration}
    ...
    \subsection{Measurements with radioactive sources}
        ...
        Then, assuming a noise rate of $\sim$\si{Hz} on the whole matrix and being the mean rate of the , the random coincidence of two hits coming from Fe-noise, Sr-noise, CR-noise and noise-noise are respectively $\sim$\SI{10}{Hz}, $\sim$\SI{2.5}{mHz}, 
        ...
        \end{figure} 
        Below I have also attached a sample of hitmap of events produced by the three different sources (fig.\ref{fig:hit_map_cosmic_rays}, \ref{fig:hit_map_Sr90}). 
        \begin{figure}[h!]
            \centering
        ...
        \red{Alle figure \ref{fig:} riporto le distribuzioni di carica rilasciata nel pixel e nel cluster in funzione del numero di pixel per evento. DISEGNO}
        The charge released by a particle which cross more pixels and do not be completely absorbed in the sensor (fig.\ref{fig:charge_sharing_cross_section}) can be described by the following relation. Considering that:
        \begin{equation}
            l = \frac{t}{cos(\lambda)} = \frac{t}{\sqrt{1+tg^2\lambda}} = \frac{t}{\sqrt{1+(x/t)^2}} 
        \end{equation}
        the charge released per space covered can be expressed as:
        \begin{equation}
            \frac{Q}{l} = \frac{Q}{t}\, \sqrt{1 \,+\, (n-1/2)^2 p^2/t^2}
        \end{equation}
        where $p/t$ is the ratio between the pitch and the epitaxial layer thickness, and then it is different in the x and y directions. Taking as value of $p/t$ 1.52, which is the mean on the two axis, the value of charge expected by the scaling relation of the charge per length covered and charge actually measured are illustrated in table \ref{tab:charge_cluster}.
        The expected value is calculated from the the measured value with two hits per cluser. The measured value has been obtained by the maximum of the distributions in the left plots in \ref{fig:spectrum}
...
%sungle pixel= [501.91226, 518.3834, 539.0669, 548.4218] pm [22.403399 22.768036 23.217813 23.418407]
        ...
        \red{Bump a sinistra dei cosmici? soglia? sto tagliando?
        Per quanto riguarda il ferro mi sarei aspettata che sommando la carica nel cluster la somma rimanesse semrpe a 1616. Questo è verificato abbastanza bene 2 e    Per quanto riguarda il ferro mi sarei aspettata che sommando la carica nel cluster la somma rimanesse semrpe a 1616. Questo è verificato abbastanza bene 2 e 3. Invece 4 ha il picco a 1850, questo significa probabilmente che quando hai 4 hit hai sia un fotone che un hit di rumore con basso tot, e che quindi ha carica circa la soglia. 
        Guardando invece allo spettro di signolo pixel si vede per i clyster da 2 e 3 un piccolo bump a 1600. Probabilnente questo corrisponde a eventi di ferro in cui una delle due hit ha tutta la carica e l'altra invece è un evento di rumore. 3. Invece 4 ha il picco a 1850, questo significa probabilmente che quando hai 4 hit hai sia un fotone che un hit di rumore con basso tot, e che quindi ha carica circa la soglia. 
        Guardando invece allo spettro di signolo pixel si vede per i clyster da 2 e 3 un piccolo bump a 1600. Probabilnente questo corrisponde a eventi di ferro in cui una delle due hit ha tutta la carica e l'altra invece è un evento di rumore.}
